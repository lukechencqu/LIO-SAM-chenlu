...
public:
    ...
    ParamServer()
    {
        ...
        nh.param<bool>("lio_sam/debugIMU", debugIMU, false);
    ...
    /* 功能：
        ...
        ——lio-sam原版使用microstrain 3dm-gx5-25 imu,其坐标系定义与ROS标准不同，为x朝后，y朝左，z朝下
        ——因此，此函数的功能是将坐标系定义与ROS标准不同的imu输出转换为ROS标准坐标系，即x朝前，y朝左，z朝上
        ——如果使用xsens imu，由于其坐标系定义符合ROS标准，因此需要将params.yaml中的变换矩阵设为单位矩阵，表示不需要变换。
            或者根本就不需要调用这个函数做多此一举的事情，但这样做显然提高了代码对传感器的通用性。
    */
